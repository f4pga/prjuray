    ...
    for tile_name in grid.tiles():
        ...
        if collisions:
            ...
            for ck in sorted(collisions):
                ...
                print("  %s: had %s, got %s" % (util.addr2str(addr, word, bit),
                                                mall[ck], mtile[ck]))
...
def run(db_root, part, verbose=False):
    ...
    db = prjuraydb.Database(db_root, part)
    db._read_tilegrid()
    '''
