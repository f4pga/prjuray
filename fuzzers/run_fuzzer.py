...
def main(argv):
    ...
    fuzzer_logdir = os.path.join(fuzzer_dir,
                                 "logs_{}".format(os.environ['URAY_PART']))
...
def run_fuzzer(fuzzer_name, fuzzer_dir, fuzzer_logdir, logger, will_retry):
    ...
    fuzzer_runok = os.path.join(fuzzer_dir,
                                "run.{}.ok".format(os.environ['URAY_PART']))
    if os.path.exists(fuzzer_runok):
        ...
        log("Skipping as run.{}.ok exists (updated @ {})",
            os.environ['URAY_PART'], last_modified.isoformat())
    ...
    if '--jobserver-fds' in make_flags or '--jobserver-auth' in make_flags:
        job_re = re.search('--jobserver-(?:fds|auth)=([0-9]+),([0-9]+)',
                           make_flags)
    ...
    if retcode != 0:
        ...
        with open(fuzzer_stdout) as f:
            stdout_log = '\n'.join(last_lines(f, 2000))

        with open(fuzzer_stderr) as f:
            stderr_log = '\n'.join(last_lines(f, 2000))
