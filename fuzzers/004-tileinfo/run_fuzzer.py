...
def start_tiles(argList):
    ...
    print("{} Running instance : {} / {}".format(datetime.datetime.now(),
                                                 blockID, total))
    subprocess.check_call(
        "${URAY_VIVADO} -mode batch -source $FUZDIR/jobtiles.tcl -tclargs " +
        str(blockID) + " " + str(start) + " " + str(stop),
        shell=True)
    print("{} Done running instance : {} / {}".format(datetime.datetime.now(),
                                                      blockID, total))
...
def main(argv):
    ...
    print("{} Generating final csv files".format(datetime.datetime.now()))
