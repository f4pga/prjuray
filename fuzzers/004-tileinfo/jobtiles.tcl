...
for {set j $start } { $j < $stop } { incr j } {
    ...
    foreach site [get_sites -quiet -of_objects $tile] {
        ...
        puts $fp "\t\t\t\"site_pins\": \["
        foreach site_pin [get_site_pins -quiet -of_objects $site] {
            # site_pin properties:
            ...
            set site_pin_node [get_nodes -quiet -of_objects $site_pin]
        ...
        puts $fp "\t\t\t\],"
        puts $fp "\t\t\t\"site_pips\": \["
        foreach site_pip [get_site_pips -quiet -of_objects $site] {
            puts $fp "\t\t\t\{"
        ...
        foreach package_pin [get_package_pins -quiet -of_objects $site] {
            puts $fp "\t\t\t\t\{"
    ...
    foreach pip [get_pips -quiet -of_objects $tile] {
        ...
        puts $fp "\t\t\t\"src_wire\":\"[get_wires -quiet -uphill -of_objects $pip]\","
